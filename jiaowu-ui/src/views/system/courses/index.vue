  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="课程编号" prop="courseNumber">
        <el-input
          v-model="queryParams.courseNumber"
          placeholder="请输入课程编号"
          clearable
    ...
    <el-table v-loading="loading" :data="coursesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="课程编号" align="center" prop="courseNumber" />
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        courseNumber: [
          { required: true, message: "课程编号，唯一，不能为空不能为空", trigger: "blur" }
        ],
        courseName: [
          { required: true, message: "课程名称，不能为空不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除存储课程信息的编号为"' + courseIds + '"的数据项？').then(function() {
