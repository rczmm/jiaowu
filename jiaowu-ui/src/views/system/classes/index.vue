  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="班级编号" prop="classNumber">
          <el-input v-model="form.classNumber" placeholder="请输入班级编号" />
        </el-form-item>
        <el-form-item label="班级名称" prop="className">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        classNumber: [
          { required: true, message: "班级编号，唯一，不能为空不能为空", trigger: "blur" }
        ],
        className: [
          { required: true, message: "班级名称，不能为空不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除存储班级信息的编号为"' + classIds + '"的数据项？').then(function() {
